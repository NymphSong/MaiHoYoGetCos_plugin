...
class HoyoBasicSpider:
    ...
    async def async_get_raw(self, params: dict) -> Response:
        """
        异步获取原始响应
    ...
    def handle_response(response: Response, is_good: bool = False) -> list:
        ...
        # 获取所有图片URL
        all_urls = [image for post in posts for image in post["post"]["images"]]
        
        # 转换为小图URL加快发送速度（米游社图片URL规则）
        # 原图: .../upload/.../xxx.jpg
        # 小图: .../upload/.../xxx__xxx.jpg 或添加缩略图参数
        optimized_urls = []
        for url in all_urls:
            # 尝试使用压缩后的图片URL（如果有的话）
            # 米游社常见的缩略图后缀处理
            if "upload" in url and "miyoushe.com" in url:
                # 使用中等质量图片，平衡速度和清晰度
                # 将原图URL转换为缩略图URL（米游社规则）
                # 例如: xxx.jpg → xxx_xxx.jpg 或其他压缩格式
    ...
    @staticmethod
    def handle_response_with_posts(response: Response, is_good: bool = False) -> list:
        """
        处理响应，返回完整帖子信息
        ...
        参数:
            ...
            - is_good: 是否精品
        返回:
            - posts: 帖子列表，每个帖子包含 subject, images, post_id 等
        """
        try:
            data = response.json()
            if "data" not in data:
            ...
            # 尝试不同的字段名（不同API可能使用不同的字段）
            data_section = data["data"]
            
            # 优先尝试 posts，然后 list
            if "posts" in data_section:
                posts = data_section["posts"]
            elif "list" in data_section:
                posts = data_section["list"]
            else:
                # 可能是直接返回的数组
                posts = data_section if isinstance(data_section, list) else []
        ...
        for post_data in posts:
            ...
            if isinstance(post_data, dict):
                ...
                result.append({
                    ...
                    "subject": post.get("subject", "无标题"),
                    "content": post.get("content", "")[:100],
                    "images": post.get("images", []),
                    "view_num": post.get("view_num", 0),
                    "like_num": post.get("like_num", 0),
                    "reply_num": post.get("reply_num", 0),
                    "author": user.get("nickname", "未知作者") if isinstance(user, dict) else "未知作者",
...
class Hot(HoyoBasicSpider):
    ...
    def get_params(self, page_size: int, offset: int = 0) -> dict:
        params = {
            "forum_id": self.forum_id,
            "is_hot": self.is_hot,
            "page_size": page_size,
    ...
    def sync_get_urls(self, page_size: int = 20, offset: int = 0) -> list:
        params = self.get_params(page_size, offset)
        return self.sync_get(params)

    async def async_get_urls(self, page_size: int = 20, offset: int = 0) -> list:
        params = self.get_params(page_size, offset)
        return await self.async_get(params)

    def sync_get_name(self, page_size: int = 20, offset: int = 0) -> list:
        params = self.get_params(page_size, offset)
        return self.sync_name(params)

    async def async_get_name(self, page_size: int = 20, offset: int = 0) -> list:
        params = self.get_params(page_size, offset)
        return await self.async_name(params)
